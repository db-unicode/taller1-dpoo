...
public class CalculadoraEstadisticas
{
	...
	// ************************************************************************
	/**
	 * Consulta el país que representa el atleta con el nombre indicado
	 * 
	 * @param nombreAtleta Nombre del atleta de quien se va a encontrar el país que representa
	 * @return el nombre del pais que representa el atleta, si el atleta no se encuentra regresa el
	 * string "NULL"
	 */
	public String obtenerPaisPorAtleta(String nombreAtleta) {
		for (Atleta unAtleta: atletas) 
		{
			String unAtletaNombre = unAtleta.darNombre();
			if (unAtletaNombre.equals(nombreAtleta)) 
			{
				return unAtleta.obtenerPaisNombre();
			}
		}
		return "NULL";
	}
